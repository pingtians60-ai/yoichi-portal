...
const STORAGE_KEY = 'yoichi_org_portal_v7';
...
class Store {
      ...
      if (resData.staff) {
        this.data.staffList = resData.staff.map(s => {
          ...
          return {
            ...
            attendance_number: (s.attendance_number !== undefined && s.attendance_number !== null) ? String(s.attendance_number).trim() : '',
  ...
  async postToGAS(action, data) {
    try {
      ...
      if (typeof Utils !== 'undefined' && Utils.showToast) {
        Utils.showToast(`同期エラー: ${e.message}`, 'error');
      }
    }
  }
  ...
  async initSupabase() {
    if (!supabaseClient) {
      console.log('Supabase is not configured. Running in LocalStorage-only mode.');
      return;
    }
    // 初回データフェッチ
  ...
  async fetchFromSupabase() {
    ...
    try {
      ...
      this.data.shopList = shopsRes.data.map(s => ({
        ...
        category: s.category,
        eventName: s.eventName,
        eventDate: s.eventDate || s.event_date || '',
        contact: s.contact,
        desc: s.desc
      }));
      ...
      this.data.staffList = staffRes.data.map(s => {
        ...
        return {
          ...
          role: s.role,
          avatar: s.avatar,
          attendance_number: (s.attendance_number !== undefined && s.attendance_number !== null) ? String(s.attendance_number).trim() : (s.attendanceNo !== undefined && s.attendanceNo !== null ? String(s.attendanceNo).trim() : ''),
          program: progs.length > 0 ? progs.join(', ') : 'なし'
        };
      });
    ...
    supabaseClient
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'yoichi_events' }, payload => {
        ...
        if (eventType === 'INSERT') {
          ...
          if (idx !== -1) this.data.events[idx] = newRow;
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'yoichi_shifts' }, payload => {
        ...
        if (eventType === 'INSERT' || eventType === 'UPDATE') {
          this.data.eventShifts[newRow.event_id] = {
            roles: newRow.roles,
            timeSlots: newRow.timeSlots,
            assignments: newRow.assignments || {}
          };
        } else if (eventType === 'DELETE') {
